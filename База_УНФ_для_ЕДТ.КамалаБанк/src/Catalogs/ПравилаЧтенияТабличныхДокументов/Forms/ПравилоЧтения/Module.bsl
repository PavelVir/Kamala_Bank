...
Процедура ТабличныйДокументВыпискиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Область)
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Массив") И ПараметрыПеретаскивания.Значение.Количество() > 0
		И ТипЗнч(ПараметрыПеретаскивания.Значение[0]) = Тип("ДанныеФормыЭлементКоллекции") Тогда

		СтандартнаяОбработка = Ложь;

		Если Область <> Неопределено Тогда
			Если ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Копирование
				И Не ПараметрыПеретаскивания.Значение[0] = НастройкиЧтения[0] Тогда
				ПараметрыПеретаскивания.Значение[0].ПоложениеПоказателя = СтрЗаменить(
				...
				ПараметрыПеретаскивания.Значение[0].ПоложениеПоказателя = ПараметрыПеретаскивания.Значение[0].ПоложениеПоказателя
					+ ?(ПустаяСтрока(ПараметрыПеретаскивания.Значение[0].ПоложениеПоказателя), "", ";") + Область.Имя;
			Иначе
				ПараметрыПеретаскивания.Значение[0].ПоложениеПоказателя = Область.Имя;
			КонецЕсли;
...
Функция СтрокаФильтраПоРасширениюФайла(ВыбранноеРасширение, ИндексВыбранногоРасширения)
	...
	ОписаниеРасширения = НСтр("ru='Книга Excel 2007 (*.xlsx)|*.xlsx'") + "|" + НСтр("ru='Книга Excel 97 (*.xls)|*.xls'")
		...
		"ru='Текстовый отчет Сбербанка (*.txt)|*.txt'");
...
Процедура ВыделитьОбластиТабличногоДокумента(Форма)
	...
	ОбластиПоказателя = МассивОбластейПоказателя(ДанныеСтроки.ПоложениеПоказателя, ДанныеСтроки.КонечнаяОбласть,
		ТабличныйДокументВыписки);
	Счетчик = 1;
	Для Каждого Область Из ОбластиПоказателя Цикл

		Суффикс = ?(ОбластиПоказателя.Количество() = 1, "", Формат(Счетчик, "ЧДЦ=; ЧГ="));
		Область.Имя         = ?(ТипЗнч(ДанныеСтроки.КлючПоказателя) = Тип("Строка"), ДанныеСтроки.КлючПоказателя,
			"Аналитика" + Формат(Счетчик, "ЧДЦ=; ЧГ=")) + Суффикс;
		Область.Примечание.Текст  = Область.Примечание.Текст + ?(Область.Примечание.Текст = "", "", Символы.ПС
			+ Символы.ПС) + ДанныеСтроки.Наименование + Символы.ПС + ДанныеСтроки.Комментарий;
...
Функция ОбработкаЗагрузкиВыписки()
	...
	Если ОбработкаЗагрузкиВыписки = Неопределено Тогда
		ОбработкаЗагрузкиВыписки = Обработки.КБ_ЗагрузкаФайла.Создать();
	КонецЕсли;
...
Функция ОбластьПоказателяКорректна(Область)
	...
	Если Область.ТипОбласти <> ТипОбластиЯчеекТабличногоДокумента.Прямоугольник Или Область.Верх <> Область.Низ
