...
Процедура ПрочитатьТабличныйДокумент(ПараметрыВызоваСервера, АдресХранилища) Экспорт
	...
	Для СчетчикСтрок = ОбластьТаблицы.Верх По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если ЗначениеЗаполнено(СтрокаВыписки.РСчет) И ЗначениеЗаполнено(СтрокаВыписки.Дата) Тогда
			Операция = НайтиОперацию(СтрокаВыписки.Дата, СтрокаВыписки.РСчет, СтрокаВыписки.СуммаПоступления, СтрокаВыписки.СуммаСписания);
		...
		лРезультат = РезультатСоответствийИзСтроки(СтрокаВыписки.НазначениеПлатежа, СтрокаВыписки.СуммаПоступления <> 0); 
		
		Если лРезультат <> Неопределено Тогда
			СтрокаВыписки.ВидОперации = лРезультат.ВидОперации;
		КонецЕсли;
		...
		И ЗначениеЗаполнено(СтрокаВыписки.Дата) И ЗначениеЗаполнено(СтрокаВыписки.РСчетПредставление);
...
Функция РезультатСоответствийИзСтроки(Знач Текст, ЭтоПоступление) 
	
	ИскомыеОбъекты = РегистрыСведений.КБ_КлючевыеСловаОбъектов.СписокОбъектовПоКлючевомуСлову(Текст, ЭтоПоступление);
