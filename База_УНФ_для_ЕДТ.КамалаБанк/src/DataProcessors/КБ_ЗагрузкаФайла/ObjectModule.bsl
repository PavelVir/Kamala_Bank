...
Функция НоваяТаблицаВыписки() Экспорт
	...
	ТипРасчетныйСчет   = Новый ОписаниеТипов("СправочникСсылка.БанковскиеСчета");
	лМассив = Новый Массив;
	лМассив.Добавить(Тип("ПеречислениеСсылка.ВидыОперацийРасходСоСчета"));
	лМассив.Добавить(Тип("ПеречислениеСсылка.ВидыОперацийПоступлениеНаСчет"));
	ТипВидОперации = Новый ОписаниеТипов(лМассив);
	...
	ТаблицаВыписки.Колонки.Добавить("Рсчет", ТипРасчетныйСчет);              // счет операции (в перемещениях - счет откуда)
	ТаблицаВыписки.Колонки.Добавить("РсчетПредставление", ТипСтрока);  // представление в таблице
	ТаблицаВыписки.Колонки.Добавить("РсчетКлючевоеСлово", ТипСтрока);  // текст, по которому сопоставляется значение
	...
	ТаблицаВыписки.Колонки.Добавить("ВидОперации", ТипВидОперации);
	...
	ТаблицаВыписки.Колонки.Добавить("НазначениеПлатежа", ТипСтрока);         // Текст, который будет записан в комментарий операции

	ТаблицаВыписки.Колонки.Добавить("НомерПП", ТипСтрока);
	
	// Дополнительные колонки
	ТаблицаВыписки.Колонки.Добавить("ВидОперацииИконка", ТипНомер);

	ТаблицаВыписки.Колонки.Добавить("РСчетКонтрагент", ТипРасчетныйСчет);
	ТаблицаВыписки.Колонки.Добавить("РСчетКонтрагентПредставление", ТипСтрока);
	ТаблицаВыписки.Колонки.Добавить("РСчетКонтрагентКлючевоеСлово", ТипСтрока);

	ТаблицаВыписки.Колонки.Добавить("ЕДРПОУКонтрагент", ТипСтрока);

	ТаблицаВыписки.Колонки.Добавить("Аналитика");
	ТаблицаВыписки.Колонки.Добавить("АналитикаПредставление");
...
Процедура ПрочитатьТабличныйДокумент(ПараметрыВызоваСервера, АдресХранилища) Экспорт
	...
	ОбластьТаблицы = ТабличныйДокумент.Область(НастройкиЧтения[0].ПоложениеПоказателя + ":"
	...
	Для Каждого СтрокаПоказателя Из НастройкиЧтения Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаПоказателя.ПоложениеПоказателя) Или СтрокаПоказателя.КлючПоказателя
			= "НачалоТаблицы" Тогда
			Продолжить;
		КонецЕсли;

		СтрокаПоказателя.ОбластиПоказателя = МассивОбластейПоказателя(СтрокаПоказателя.ПоложениеПоказателя,
			СтрокаПоказателя.КонечнаяОбласть, ТабличныйДокумент);
	...
	ТабличныйДокумент.Область(ОбластьТаблицы.Верх, 1, ТабличныйДокумент.ВысотаТаблицы,
		ТабличныйДокумент.ШиринаТаблицы).Разъединить();
	...
	Для СчетчикСтрок = ОбластьТаблицы.Верх По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если Не ЗначениеЗаполнено(ТекстДаты) Или Не ЗначениеЗаполнено(ЗначениеДаты) Или ТипЗнч(ЗначениеДаты) <> Тип(
			"Дата") Тогда
		...
		УстановитьЗначениеВСтрокеВыписки(СтрокаВыписки, "Рсчет", НастройкиЧтения, ТабличныйДокумент);
		УстановитьЗначениеВСтрокеВыписки(СтрокаВыписки, "НомерПП", НастройкиЧтения, ТабличныйДокумент);
		УстановитьЗначениеВСтрокеВыписки(СтрокаВыписки, "НазначениеПлатежа", НастройкиЧтения, ТабличныйДокумент);

		УстановитьЗначениеВСтрокеВыписки(СтрокаВыписки, "РСчетКонтрагент", НастройкиЧтения, ТабличныйДокумент);
		УстановитьЗначениеВСтрокеВыписки(СтрокаВыписки, "ЕДРПОУКонтрагент", НастройкиЧтения, ТабличныйДокумент);
		
		// Суммы поступления и списания прорабатываем отдельно
		СуммаПоступления = ЗначениеИПредставлениеПоказателя(СтрокаВыписки, "СуммаПоступления", НастройкиЧтения,
			ТабличныйДокумент);
		СуммаСписания    = ЗначениеИПредставлениеПоказателя(СтрокаВыписки, "СуммаСписания", НастройкиЧтения,
			ТабличныйДокумент);

		Если СуммыВОднойКолонке Тогда
			
			// поступление или списание - определяется знаком суммы
			СтрокаВыписки.СуммаПоступления              = ?(ЗнакЧисла(СуммаПоступления.Значение) > 0,
				СуммаПоступления.Значение, 0);
			СтрокаВыписки.СуммаПоступленияКлючевоеСлово = ?(ЗнакЧисла(СуммаПоступления.Значение) > 0,
				СуммаПоступления.Представление, "");

			СтрокаВыписки.СуммаСписания                 = ?(ЗнакЧисла(СуммаСписания.Значение) < 0,
				-СуммаСписания.Значение, 0);
			СтрокаВыписки.СуммаСписанияКлючевоеСлово    = ?(ЗнакЧисла(СуммаСписания.Значение) < 0,
				СуммаСписания.Представление, "");
		...
		Если СтрокаВыписки.СуммаПоступления = СтрокаВыписки.СуммаСписания Тогда
			ВызватьИсключение "Не реализовано загрузку перемещение";
				///СтрокаВыписки.СсылкаНаОперацию         = Документы.Перемещение.ПустаяСсылка();

		ИначеЕсли
		СтрокаВыписки.СуммаПоступления > 0 Тогда

			СтрокаВыписки.СсылкаНаОперацию         = Документы.ПоступлениеНаСчет.ПустаяСсылка();

		Иначе

			СтрокаВыписки.СсылкаНаОперацию         = Документы.РасходСоСчета.ПустаяСсылка();

		КонецЕсли;
		СписокАналитики = РезультатСоответствийИзСтроки(СтрокаВыписки.НазначениеПлатежа);

		Если СписокАналитики <> Неопределено Тогда
			СтрокаВыписки.Аналитика = НоваяТаблицаАналитики();

			Для Каждого ЭлементАналитики Из СписокАналитики Цикл

				Если ЗначениеЗаполнено(ЭлементАналитики.ВидОперации) Тогда
					СтрокаВыписки.ВидОперации = ЭлементАналитики.ВидОперации;
				КонецЕсли;

				Если ЗначениеЗаполнено(ЭлементАналитики.Значение) Тогда
					СтрокаАналитики = СтрокаВыписки.Аналитика.Добавить();
					СтрокаАналитики.ВидАналитики =  ЭлементАналитики.ИдентификаторОбъекта;
					СтрокаАналитики.КлючевоеСлово = ЭлементАналитики.КлючевоеСлово;
					СтрокаАналитики.Представление = ЭлементАналитики.Представление;
					СтрокаАналитики.Значение      = ЭлементАналитики.Значение;

					СтрокаВыписки.АналитикаПредставление = "" + СтрокаВыписки.АналитикаПредставление + ?(
						ЗначениеЗаполнено(
					СтрокаВыписки.АналитикаПредставление), "; ", "") + Строка(СтрокаАналитики.ВидАналитики) + " = "
						+ Лев(СтрокаАналитики.Представление, 30);
		...
		СтрокаВыписки.Загружать = Не ЗначениеЗаполнено(СтрокаВыписки.СсылкаНаОперацию) И ЗначениеЗаполнено(
			СтрокаВыписки.Дата) И ЗначениеЗаполнено(СтрокаВыписки.РСчетПредставление);
...
Процедура ЗагрузитьCSVФайлВТаблицу(ИмяФайла, Таблица) Экспорт
	...
	Если (ПервыйСимвол = """" Или ПервыйСимвол = "'" Или ПервыйСимвол = "*") И ВхожденийСимвола >= МинимумПовторов * 2 Тогда

		КавычкаОткрытая = ПервыйСимвол;
		КавычкаЗакрытая = ПервыйСимвол;

	ИначеЕсли ПервыйСимвол = "[" И ВхожденийСимвола >= МинимумПовторов И СтрЧислоВхождений(Строка, "]")
		>= МинимумПовторов Тогда
	...
	Если СимволРазделителя = Неопределено Тогда
		ТекстСообщения = НСтр(
			"ru = 'В указанном файле не удалось определить разделитель показателей. Убедитесь в корректности данных в файле. Убедитесь в корректности данных в файле.'");
		Возврат;
	КонецЕсли;
	...
	Пока СтрокаТекста <> Неопределено Цикл
			...
			Если СтрокаЗаголовка = Неопределено Тогда
				Для Каждого Колонка Из СписокКолонок Цикл
					...
					Если Не ПустаяСтрока(ТекстЯчейки) Тогда
						...
						Колонка.СтроковоеЗначение = Колонка.СтроковоеЗначение + ?(Колонка.СтроковоеЗначение = "", "",
							" ") + ТекстЯчейки;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		ИначеЕсли СостояниеЧтения = "ЧтениеТаблицы" Тогда
			...
			Если ЗначениеЗаполнено(СтрокаОписанияСчета) Тогда
				...
				ТекущийСчет.ОписаниеСчета = ТекущийСчет.ОписаниеСчета + ?(ПустаяСтрока(ТекущийСчет.ОписаниеСчета), "",
					" ") + СтрокаОписанияСчета;
			...
			СтрокаОписанияДаты = СокрЛП(Сред(СтрокаТекста, СписокКолонок[2].НачальнаяПозиция,
				СписокКолонок[2].КоличествоСимволов));
			...
			Для Счетчик = 1 По КоличествоКолонок - 1 Цикл
				...
				ТекстЯчейки = СтрЗаменить(СокрЛП(Сред(СтрокаТекста, СтрокаКолонки.НачальнаяПозиция,
					СтрокаКолонки.КоличествоСимволов)), "   ", " ");
				Если ЗначениеЗаполнено(ТекстЯчейки) Тогда
					СтрокаКолонки.СтроковоеЗначение = СтрокаКолонки.СтроковоеЗначение + ?(
						СтрокаКолонки.СтроковоеЗначение = "", "", " ") + ТекстЯчейки;
				КонецЕсли;
...
Процедура ЗаписатьЗначенияЯчеекВСтрокуВыписки(ТекущаяСтрокаВыписки, СписокКолонок,
	ОчищатьСтроковыеЗначенияКолонок = Истина) 
	...
	ТекущаяСтрокаВыписки.ДатаОперации       = ДатаИзЧислаИМесяцаСтрокой(СписокКолонок[1].СтроковоеЗначение, Год(
		ТекущаяСтрокаВыписки.ДатаОбработки));
...
Функция НоваяТаблицаАналитики()
	...
	Результат.Колонки.Добавить("ВидАналитики", Новый ОписаниеТипов("СправочникСсылка.ИдентификаторыОбъектовМетаданных"));
...
Процедура УстановитьЗначениеВСтрокеВыписки(СтрокаВыписки, КлючПоказателя, НастройкиЧтения, ТабличныйДокумент)
	...
	лМассивИсключаемыхПолей = Новый Массив;
	лМассивИсключаемыхПолей.Добавить("НазначениеПлатежа");
	лМассивИсключаемыхПолей.Добавить("НомерПП");
	лМассивИсключаемыхПолей.Добавить("ЕДРПОУКонтрагент");

	ЗначениеИПредставление = ЗначениеИПредставлениеПоказателя(СтрокаВыписки, КлючПоказателя, НастройкиЧтения,
		ТабличныйДокумент);
	СтрокаВыписки[КлючПоказателя]                   = ЗначениеИПредставление.Значение;

	Если лМассивИсключаемыхПолей.Найти(КлючПоказателя) = Неопределено Тогда
		СтрокаВыписки[КлючПоказателя + "КлючевоеСлово"]    = ЗначениеИПредставление.КлючевоеСлово;

		ДобавитьПредставление = КлючПоказателя <> "ОписаниеОперации" И Не ТипЗнч(ЗначениеИПредставление.Значение) = Тип(
			"Дата") И Не ТипЗнч(ЗначениеИПредставление.Значение) = Тип("Число");
...
Функция ЗначениеИПредставлениеПоказателя(СтрокаВыписки, КлючПоказателя, НастройкиЧтения, ТабличныйДокумент)
	...
	Если Не ЗначениеЗаполнено(НастройкаПокзателя.ПоложениеПоказателя) Тогда
		...
		Результат.Значение = ЗначениеПоказателяИзТекста("", НастройкаПокзателя.ТипЗначенияПоказателя,
			НастройкаПокзателя.КлючПоказателя, ЭтоПоступление);
		Возврат Результат;

	КонецЕсли;
		
	// Получаем текстовое представление значения показателя
	Результат.КлючевоеСлово = ТекстПоказателяЯчеекПоНомеруСтроки(СтрокаВыписки.НомерСтрокиИсходника,
		НастройкаПокзателя.ОбластиПоказателя, ТабличныйДокумент);

	Если ЗначениеЗаполнено(НастройкаПокзателя.ТекстПоследнегоЗначения) И НастройкаПокзателя.ТекстПоследнегоЗначения
		= Результат.КлючевоеСлово И НастройкаПокзателя.ЭтоПоступление = ЭтоПоступление Тогда
	...
	Иначе
		...
		Результат.Значение      = ЗначениеПоказателяИзТекста(Результат.КлючевоеСлово,
			НастройкаПокзателя.ТипЗначенияПоказателя, НастройкаПокзателя.КлючПоказателя, СтрокаВыписки.СуммаПоступления
			<> 0);
	...
	Результат.Представление = ?(ЗначениеЗаполнено(Результат.Значение), Строка(Результат.Значение),
		Результат.КлючевоеСлово);
	...
	Для Каждого Область Из МассивОбластейПоказателя Цикл
		...
		Иначе
			...
			ТекстПоказателя = ТекстПоказателя + ?(ТекстПоказателя = "", "", " ") + ТабличныйДокументВыписки.Область(
				НомерСтроки, Область.Лево).Текст;
		КонецЕсли;
...
Процедура ДополнитьТекстСтрокиВыписки(НомерСтроки, СтрокаВыписки, ТабличныйДокумент, ОбластьТаблицы,
	ПропускатьПустыеКолонки)
...
Функция ДатаИзЧислаИМесяцаСтрокой(Знач Текст, Знач Год = 0)
	...
	Число = ?(ЗначениеЗаполнено(ЧислоСтрокой) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЧислоСтрокой), Число(
		ЧислоСтрокой), 0);
...
Функция ДатаИзСтроки(Знач Текст)
	...
	Если МассивСлов.Количество() = 2 И Найти(МассивСлов[1], "9") = 1 Тогда

		Число = 1;
		ПозицияТочки = СтрДлина(МассивСлов[0]) + 1;

		МесяцСтрокой = Лев(Текст, ПозицияТочки - 1);
		Если Найти(МассивСлов[0], "X") = 1 Тогда
			// ИмяМесяца.Год
			Месяц = НомерИзИмениМесяца(МесяцСтрокой);
		Иначе
			// НомерМесяца.Год
			Месяц = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(МесяцСтрокой);
		КонецЕсли;

		Год   = НомерГодаИзТекста(Сред(Текст, ПозицияТочки + 1));

	ИначеЕсли МассивСлов.Количество() = 3 И Найти(МассивСлов[0], "9") = 1 И Найти(МассивСлов[2], "9") = 1 Тогда
		ПозицияТочки1 = СтрДлина(МассивСлов[0]) + 1;
		ПозицияТочки2 = ПозицияТочки1 + СтрДлина(МассивСлов[1]) + 1;

		Число        = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Лев(Текст, ПозицияТочки1 - 1));

		МесяцСтрокой = Сред(Текст, ПозицияТочки1 + 1, СтрДлина(МассивСлов[1]));
		Если Найти(МассивСлов[1], "X") = 1 Тогда
			// Число.ИмяМесяца.Год
			Месяц = НомерИзИмениМесяца(МесяцСтрокой);
		Иначе
			// Число.НомерМесяца.Год
			Месяц = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(МесяцСтрокой);
		КонецЕсли;

		Год          = НомерГодаИзТекста(Сред(Текст, ПозицияТочки2 + 1, СтрДлина(МассивСлов[2])));
...
Функция РезультатСоответствийИзСтроки(Знач Текст)
	...
	Возврат РегистрыСведений.КБ_КлючевыеСловаОбъектов.СписокОбъектовПоКлючевомуСлову(Текст);
